...
import org.javacoo.cowswing.plugin.kbs.ui.model.KbsTypeDefaultTreeCellRenderer;
 ...
 * @version 1.0
 */
public abstract class AbstractTypeTreePanel extends JPanel implements ITypeTree{
	...
	private void initCmp(){
		...
		tree.setCellRenderer(new KbsTypeDefaultTreeCellRenderer());
	...
	public void expandTree(boolean expandAll) {
		KbsTypeTreeNode root = (KbsTypeTreeNode) tree.getModel().getRoot();
	...
	private DefaultTreeModel createRootTreeModel() {
		...
		root.setTypeCode(KbsConstant.KBS_TYPE_ROOT_CODE);
		root.setTypeName(LanguageLoader.getString("Kbs.type_root_name"));
		KbsTypeTreeNode rootNode = new KbsTypeTreeNode(root, kbsTypeService);
		rootNode.add(createChildNode(KbsConstant.KBS_TYPE_ROOT_CODE,KbsConstant.KBS_TYPE_CHILD_DEFAULT_CODE,LanguageLoader.getString("Kbs.type_child_default_name")));
		rootNode.add(createChildNode(KbsConstant.KBS_TYPE_ROOT_CODE,KbsConstant.KBS_TYPE_CHILD_ADDRESS_CODE,LanguageLoader.getString("Kbs.type_child_address_name")));
		rootNode.add(createChildNode(KbsConstant.KBS_TYPE_ROOT_CODE,KbsConstant.KBS_TYPE_CHILD_BOOKMARK_CODE,LanguageLoader.getString("Kbs.type_child_bookmark_name")));
		rootNode.explore();
	 ...
	 * @return 子节点
	 */
	private KbsTypeTreeNode createChildNode(String parentTypeCode,String typeCode,String typeName){
		...
		return new KbsTypeTreeNode(child, kbsTypeService);
	}
	/**
	 * 展开
	 * <p>方法说明:</>
	...
	private void expand(JTree tree, TreePath parent, boolean expand) {
		KbsTypeTreeNode node = (KbsTypeTreeNode) parent.getLastPathComponent();
		if (node.getChildCount() >= 0) {
			for (Enumeration e = node.children(); e.hasMoreElements();) {
				KbsTypeTreeNode n = (KbsTypeTreeNode) e.nextElement();
				n.explore();
