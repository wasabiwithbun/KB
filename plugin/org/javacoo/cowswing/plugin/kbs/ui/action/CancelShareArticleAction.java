 ...
 * @version 1.0
 */
@Component("cancelShareArticleAction")
public class CancelShareArticleAction  extends AbstractAction{
	...
	public CancelShareArticleAction(){
		super(LanguageLoader.getString("Kbs.module_article_cancle_share_btn"),ImageLoader.getImageIcon("CrawlerResource.kbs_article_cancel_share"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(moduleArticleTable.getSelectedRow() != -1){
			int result = JOptionPane.showConfirmDialog(null, LanguageLoader.getString("Kbs.module_article_cancel_share_info"),LanguageLoader.getString("Common.confirm"), JOptionPane.YES_NO_OPTION); 
			if(result == 0){
	...
	private void shareByArticleIdList(List<Integer> idList){
		...
		kbsModuleArticleBean.setPurview(KbsConstant.SEARCH_PURVIEW_PERSON);
	...
	private MsgBean populateArticleMsg(KbsModuleArticleBean articleBean) {
		...
		msgBean.setPurview(KbsConstant.SEARCH_PURVIEW_PERSON);
